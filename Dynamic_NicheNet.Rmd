...
hepa_cluster <- read.csv("C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/DE_pseudobulk_cluster.csv", row.names = 1)
```
...
sender cell type, receiver cell type. We will analyze 7x7=49 different combinations
...
receiver_celltype <- "endothelial"
sender_timepoint <- "12_phase_2"
...
# Disappeared genes
# 93 genes -> 63 genes (old version)
# 96 genes -> 85 genes (new version)
...
# Rank the prediction
ligand_activities %>% arrange(-aupr) # PCC best explains ligand activity
best_upstream_ligands = ligand_activities %>% top_n(20, aupr) %>% arrange(-aupr) %>% pull(test_ligand)
...
plot_trend <- function(interesting_genes, celltype){
  ...
  for(gene in interesting_genes) {
    ENSEMBL <- row.names(hepa_cluster)[which(hepa_cluster$Symbol == gene & hepa_cluster$celltype == celltype)]
    logCPM.obs.i <- logCPM.obs[ENSEMBL,]
    logCPM.fit.i <- logCPM.fit[ENSEMBL,]
    plot(hours, logCPM.obs.i, ylab="log-CPM", main=gene, pch=16)
...
```{r}
gene <- "Wnt5b"
ENSEMBL <- row.names(hepa_cluster)[which(hepa_cluster$Symbol == gene & hepa_cluster$celltype == sender_celltype)]
ENSEMBL_celltype <- paste0(ENSEMBL, ".", sender_celltype)
logCPM.obs.i <- logCPM.obs[ENSEMBL_celltype,]
logCPM.fit.i <- logCPM.fit[ENSEMBL_celltype,]
plot(hours, logCPM.obs.i, ylab="log-CPM", main=paste0(gene, ".", sender_celltype), pch=16)
lines(hours, logCPM.fit.i, col="red", lwd=2)
...
**Active Ligands**
```{r}
plot_trend(active_ligand_target_links_df$ligand, sender_celltype)
```

**Active Targets**
```{r}
plot_trend(active_ligand_target_links_df$target)
```
