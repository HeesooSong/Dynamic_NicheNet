...
tab <- as.data.frame(topTags(fit, n=1498)) #15256
head(tab)
```
...
tab$Symbol[which(is.na(tab$Symbol))] <- tab$ENSEMBL[which(is.na(tab$Symbol))] #fill in NA with ENSEMBL ID
...
for (n in 1:length(sig_norm$gene)) {
  gene_symbol[n] <- tab$Symbol_celltype[which(rownames(tab) == sig_norm$gene[n])]
}
```

```{r}
# There are multiple Mup13 in gene name
# ENSMUSG00000078675, ENSMUSG00000096688

gene_symbol[duplicated(gene_symbol)]
```
```{r}
rownames(sig_norm) <- sig_norm[,"gene"]
```

### Calculate distance & cluster genes
```{r}
gene_dist <- dist(sig_norm[, 2:length(colnames(sig_norm))])
gene_hclust <- hclust(gene_dist, method = "complete")
```

```{r}
gene_cluster <- data.frame(cutree(gene_hclust, k = 50)) %>%
  rownames_to_column()
