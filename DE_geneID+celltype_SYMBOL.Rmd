...
cluster_counts["order"] <- c(1:length(rownames(cluster_counts)))
```
...
cluster_counts2 <- subset(cluster_counts2, select=-c(order, ENSEMBL, celltype, Symbol)) ####


cluster_counts <- cluster_counts[order(cluster_counts$order),]
cluster_counts <- subset(cluster_counts, select=-c(order, ENSEMBL, celltype, Symbol))
...
y2 <- DGEList(counts = cluster_counts2, group = hours)

```

```{r}
# filter out lowly expressed genes. This solves(?) row order problem.
keep <- filterByExpr(y)
y <- y[keep, , keep.lib.sizes = FALSE]

keep2 <- filterByExpr(y2)
...
sig_num2 <- summary(decideTests(fit2))
sig_num2
```
...
```{r}
# Genes with most significant time effects
tab2 <- as.data.frame(topTags(fit2, n=dim(fit2)[1]))
head(tab2)
```
...
```{r}
#write.table(tab, file = "C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/tab.csv", sep = ",")
```
...
logCPM.obs2 <- edgeR::cpm(y2, log=TRUE, prior.count=fit2$prior.count)
logCPM.fit2 <- edgeR::cpm(fit2, log=TRUE)
```
...
normalized_counts2 <- logCPM.fit2
...
sig_norm2 <- data.frame(normalized_counts2) %>%
  rownames_to_column(var = "gene") %>%
  dplyr::filter(gene %in% rownames(tab2)[1:sig_num2[2]])

colnames(sig_norm2) <- c("gene", experiments)
sig_norm2 <- sig_norm2 %>% column_to_rownames(var="gene")
head(sig_norm2)

# only difference is the order of the genes.
```
```{r}
alp_signorm <- sig_norm[order(row.names(sig_norm)),]
alp_signorm2 <- sig_norm2[order(row.names(sig_norm2)),]
```
...
```{r}
gene_dist2 <- Dist(alp_signorm, method="spearman")
gene_hclust2 <- hclust(gene_dist2, method = "ward.D2")
```

```{r}
gene_dist3 <- Dist(alp_signorm2, method="spearman")
gene_hclust3 <- hclust(gene_dist3, method = "ward.D2")
```
...
plot(gene_hclust2, labels=FALSE, hang=-1)
plot(gene_hclust3, labels=FALSE, hang=-1)
```
...
```{r}
gene_cluster2 <- data.frame(cutree(gene_hclust2, k = 4)) %>%
  rownames_to_column()
colnames(gene_cluster2) <- c("gene", "cluster_number")

gene_cluster3 <- data.frame(cutree(gene_hclust3, k = 4)) %>%
  rownames_to_column()
colnames(gene_cluster3) <- c("gene", "cluster_number")
```
...
histogram2 <- c()
for(i in 1:4) {
  cluster_genes2 <- gene_cluster2[gene_cluster2$cluster_number == i, "gene"]
  histogram2 <- c(histogram2, length(cluster_genes2))
}

histogram3 <- c()
for(i in 1:4) {
  cluster_genes3 <- gene_cluster3[gene_cluster3$cluster_number == i, "gene"]
  histogram3 <- c(histogram3, length(cluster_genes3))
}
...
plot(histogram2, type="h", lwd=10, ylim=c(0, 6000), xlab="Cluster", ylab="Cluster Size")
text(histogram2, format(histogram2), pos=3, col="red", cex=0.8)

plot(histogram3, type="h", lwd=10, ylim=c(0, 6000), xlab="Cluster", ylab="Cluster Size")
text(histogram3, format(histogram3), pos=3, col="red", cex=0.8)
```
...
cluster_number_merged2 <- merge(x = tab[1:sig_num[2],], y = gene_cluster2, by.x = 0 , by.y = "gene", all = TRUE)
...
#write.table(cluster_number_merged2, file="C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/DE_pseudobulk_cluster_difforder.csv", sep = ",")
```
