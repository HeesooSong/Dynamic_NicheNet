...
cluster_counts2 <- subset(cluster_counts2, select=-c(ENSEMBL, celltype, Symbol, order)) ####


cluster_counts <- cluster_counts[order(cluster_counts$order),]
cluster_counts <- subset(cluster_counts, select=-c(ENSEMBL, celltype, Symbol, order))
...
PooledCounts <- sumTechReps(cluster_counts, ID = hours)
PooledCounts2 <- sumTechReps(cluster_counts2, ID = hours)
```
...
```{r}
y <- DGEList(counts = PooledCounts, group = unique(hours))
y2 <- DGEList(counts = PooledCounts2, group = unique(hours))
...
```{r}
y2 <- estimateDisp(y2, design) # reduced from 134596 genes -> 92949 genes
...
tab2 <- as.data.frame(topTags(fit2, n=90000))
head(tab2)
```

```{r}
# Genes with most significant time effects
tab <- as.data.frame(topTags(fit, n=90000))
head(tab)
```
```{r}
all.equal(tab, tab2)
