...
```{r cache=TRUE}
# #hepa_cluster <- read.csv("C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/DE_pseudobulk_cluster_phase2.csv")
...
cluster_number <- 1
main_path <- "C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/Dynamic_NicheNet"
dir.create(file.path(main_path, paste0("Dynamic_NicheNet_DecontX_", sender_celltype, "_", receiver_celltype, "_", cluster_number)))
vis_path <- paste0(main_path, "/Dynamic_NicheNet_DecontX_", sender_celltype, "_", receiver_celltype, "_", cluster_number, "/")
...
for (i in 1:10){
  ...
  pdf(file = paste(vis_path, ligand, "-", target, "combined_trend.pdf", sep=""), width = 12, height = 7)
  plot_combined_trends_ggplot2(celltype_combinations, logCPM.obs, rownames(hepa_cluster))
  dev.off()
}
```
...
```{r}
# Check if the gene is "expressed" in a cell type of a sample
# gene <- "Lmnb1"
# for (timepoint in experiments){
...
#       ifelse(!(gene %in% expressed_genes_receiver_onetype), print(paste0(gene, " not in ", receiver_celltype, " at ", timepoint)), NA)
#   }
# }
```

```{r}
# # Check expression of the gene through time points
# gene <- "Lmnb1"
# ENSEMBL <- "ENSMUSG00000024590"
# 
# gene_subset <- subset(x = hepa_celltypeOI, subset = ENSMUSG00000024590 >= 1)
# 
...
```{r}
# Check expression "level" of the gene through time points
