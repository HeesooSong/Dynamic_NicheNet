---
title: "DE_geneID+celltype_excl48Phase1"
author: "Heesoo Song"
date: '2022 1 11 '
output: html_document
---

Here, we try psuedobulk analysis excluding the timepoint 48h_phase1 as this has high difference in gene expression level with 48h_phase2. This may be hindering the correct fitting of the model. The difference of features in 48h_phase1 and 48h_phase2 can be seen in MDS plot of *DE_geneID+celltype.Rmd*. 
...
experiments <- c("0_phase_1", "3_phase_2", "6_phase_2", "12_phase_2", "24_phase_2", "36_phase_2", "48_phase_2", "72_phase_2", "96_phase_2", "120_phase_2", "168_phase_2")
```
...
```{r}
hours <- c(0, 3, 6, 12, 24, 36, 48, 72, 96, 120, 168)
```
...
```{r}
#write.table(rownames(sig_norm), file = "C:/Users/pc/Desktop/MastersProject/Dynamic_NicheNet/sig_genes_no48h-2.csv", sep = ",")
```
...
#   logCPM.fit.i <- data.frame(logCPM.fit[cluster_genes,])
#   
#   if (length(cluster_genes) > 1) {
#     plot(hours, logCPM.fit.i[1,], type = "l", ylab="log-CPM", main=paste0("cluster ", i, ": ", length(cluster_genes), " genes"), pch=16)
#     for (j in 2:length(cluster_genes)) {
#       lines(hours, logCPM.fit.i[j,], lwd=2)
#     }
#   } else {
#     plot(hours, t(logCPM.fit.i), type = "l", ylab = "log_CPM", main=paste0("cluster ", i, ": 1 gene"), pch=16)
#   }
# }
```
...
for (i in 1:length(FlybaseID_list)) {
  ...
  print(VlnPlot(hepa_clusterOI, group.by = "time_experiment", features = feature_OI[1], slot = "counts", pt.size = 0) + 
          geom_boxplot(width=0.1, fill="white") + 
          labs(title = interesting_genes[i]))
}
```
